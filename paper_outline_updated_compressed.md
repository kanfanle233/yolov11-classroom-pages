...
**论文主线**  
本文研究真实智慧课堂中“视觉行为片段是否与课堂语义事件一致”的验证问题。系统先用 YOLOv11s 行为检测器和 YOLO pose 构建学生级视觉行为序列，再从 Whisper ASR 和事件抽取模块得到文本语义流。随后，系统用语义桥接协议统一视觉短码和文本事件，用轨迹不确定性调节跨模态时间窗口，并通过动态加权的双重验证输出 `match`、`mismatch` 或 `uncertain`。最终结果以学生级时间线和证据面板呈现，支持教师或研究者审计模型判断。

**核心写作原则**

- YOLOv11s 是基础检测器，不写成本文创新点。
- ASR 是有质量门控的文本来源，不写成稳定提升模块。
- LLM 相关模块当前只能写为原型或未来扩展。
...
智慧课堂行为分析不仅需要识别学生动作，还需要判断这些动作是否与课堂语义事件相互印证。单一视觉检测在遮挡、远距离、多人重叠和小动作相似的场景中容易产生不稳定判断；语音转写虽然提供了教师指令和课堂活动的语义线索，但远场噪声、回声和 ASR 错识别也可能引入不可靠文本证据。为解决这一问题，本文提出一个面向智慧课堂视觉行为序列与文本语义流的可审计双重验证框架。系统使用 YOLOv11s 行为检测器和 YOLO pose 构建 8 类课堂行为片段与学生轨迹，并基于关键点可见率、运动稳定性和边界框稳定性估计轨迹不确定性。随后，系统通过语义桥接协议将视觉短码转换为可验证的中英文语义标签，在文本侧使用 Whisper ASR、质量门控和事件查询抽取构建语义流，并在文本不可靠时触发视觉回退。跨模态对齐阶段根据运动状态和轨迹不确定性自适应调整候选时间窗口；验证阶段根据视觉分数、文本分数、动态模态权重和不确定性惩罚输出三值决策，并保留完整证据链。当前本地实验表明，该框架能够生成学生级时间线、事件查询、对齐候选、验证事件和 VSumVis 风格可视分析界面。行为检测主线已有验证集证据支持，但独立测试集划分和事件级人工金标仍需补强。因此，本文的主要贡献在于把课堂视觉检测结果转化为可审计、可解释、可交互分析的事件级结果，而不是提出新的检测器结构。
...
| 4. Framework and Method | 1.7 | 语义桥接、UQ 对齐、动态验证 |
| 5. Visual Analytics System | 1.1 | 设计目标、视图、交互、审计任务 |
| 6. Experiments and Results | 1.6 | 检测、pipeline、ASR gate、消融 |
...
本文的贡献可概括为四点。第一，提出一个面向智慧课堂的视觉行为序列与文本语义流双重验证框架，将检测结果转化为可审计事件。第二，设计语义桥接协议，使视觉短码能够被文本查询、验证器和时间线界面共同使用。第三，引入不确定性感知的自适应时间对齐和动态可靠性评分，在 ASR 可用、低质量或缺失时保持验证链路稳定。第四，构建学生级时间线与证据链可视分析界面，使教师和研究者能够检查模型判断的来源。
...
本节是方法核心。写作时要围绕“从检测到验证”的证据链展开，而不是罗列 pipeline 脚本。建议按四个模块写：视觉行为序列、语义桥接、UQ 对齐、双重验证。
...
## 4.2 Semantic Bridging Protocol
...
## 4.3 Track Uncertainty Estimation
...
- 验证：`verifier/infer.py`、`verifier/model.py`。
...
系统实现使用 YOLOv11s 行为检测器、YOLO pose、tracking and smoothing、Whisper ASR、事件查询抽取、自适应对齐、dual verifier 和 D3/FastAPI 可视分析界面。主流程由 `scripts/main/09_run_pipeline.py` 编排。
...
在验证输出方面，`front_45618_full` 包含 `align_multimodal.json` 和 `verified_events.jsonl`。报告中一个示例事件具有 `p_match=0.7132`、`reliability_score=0.6603`、`uncertainty=0.3397` 和 `label=match`。这说明系统已经能保留事件级证据链。由于事件级人工金标仍少，正式统计指标应以补充标注后的结果为准。
...
第四，LLM、OCR、MLLM 和若干自定义检测模块尚未形成主线实验证据。它们应作为 future work，而不是当前贡献。

第五，当前系统是离线或准离线 pipeline 加本地 FastAPI 原型。生产级实时部署、权限管理、跨教室泛化和隐私治理仍需进一步研究。

## 8.2 Conclusion

本文提出了一个面向智慧课堂视觉行为序列与文本语义流的可审计双重验证框架。该框架把 YOLOv11s 行为检测、pose tracking、ASR 质量门控、语义桥接、不确定性感知对齐和动态可靠性评分组织成一条完整证据链，并通过学生级时间线和可视分析界面呈现事件判断。当前本地实验表明，该系统能够生成可追溯的事件查询、对齐候选、验证标签和时间线输出，为噪声课堂场景下的多模态行为分析提供了可行基础。未来工作将重点补充视频级独立测试集、更大规模人工事件标注、跨场景验证和更强的学习式 verifier。
